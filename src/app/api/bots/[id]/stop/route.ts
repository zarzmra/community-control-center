...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const desiredStatus = requestedDesiredStatus("stop");
    ...
    const operation = await withTransaction(async (client) => {
      ...
      const inserted = await client.query<BotOperation>(
        ...
        RETURNING *`,
        [id, current.community_id, "stop" as BotOperationType, session.userId, correlationId],
      );
      ...
      await recordAuditLog(
        ...
        `Se registró la operación stop para el bot "${current.name}"; correlation_id=${correlationId}; desired_status=${desiredStatus}`,
    ...
    return NextResponse.json({
      ...
      message: "La operación de detención quedó registrada y pendiente de ejecución por un runtime.",
    });
  } catch (error) {
    return apiErrorResponse(error, "No se pudo detener el bot.");
  }
}
