...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    if (bot.status !== "running") {
      throw new ApiError(409, "Solo se puede reiniciar un bot en ejecución.");
    }

    const nextStatus = transitionBotStatus(bot.status, "stopping");
    const desiredStatus = requestedDesiredStatus("restart");
    ...
    const operation = await withTransaction(async (client) => {
      ...
      const inserted = await client.query<BotOperation>(
        ...
        RETURNING *`,
        [id, current.community_id, "restart" as BotOperationType, session.userId, correlationId],
      );
      ...
      await recordAuditLog(
        ...
        `Se registró la operación restart para el bot "${current.name}"; correlation_id=${correlationId}; desired_status=${desiredStatus}`,
    ...
    return NextResponse.json({
      ...
      message: "La operación de reinicio quedó registrada como una orden lógica pendiente de ejecución por un runtime.",
    });
  } catch (error) {
    return apiErrorResponse(error, "No se pudo reiniciar el bot.");
  }
}
